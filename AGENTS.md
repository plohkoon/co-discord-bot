...
**Pending-application lifecycle:** one pending application per person×team, enforced three deep (the `/team apply` pre-check, `Applications::Submit`'s `DuplicatePending`, and a partial unique index). `Applications::Sweep` (via the recurring `ApplicationSweepJob`, config/recurring.yml, every 15 min) posts escalating officer reminders in the review channel at 24h/3d/6d (`team_applications.reminder_stage` tracks what's been sent; missed windows collapse into one catch-up) and auto-rejects at 7 days through the same `Decide` path as the buttons — `decided_by_discord_id: nil` means the system decided, and the review message is repainted over REST.
