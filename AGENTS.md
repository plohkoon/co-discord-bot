...
**Team roster/directory:** teams carry free-form roster lines (`Team::ROSTER_FIELDS`: team_type, progression, requirements, date_and_time, current_needs) plus an optional `TeamCategory` (directory section header, `locate` find-or-creates per guild, position-ordered). Set them via `/team create|edit` options or the web team page. `/team roster` (admin) posts the directory — a header message per category, one message per team (`CoBot::RosterMessage`) with an Apply button (`Components::ApplyButton`, same modal flow as `/team apply` via the shared `Commands::ApplyFlow`); leads are the officer role's current holders, mentions never ping (`allowed_mentions: {parse: []}`). Each team records roster_channel_id/message_id at post time; any team edit (bot or web) enqueues `TeamRosterRefreshJob` to repaint that block over REST (leads re-derived by member pagination; a deleted post clears the stored ids).
